...
const CanceledBookings = () => {
  ...
  // Pagination logic
  const totalPages = Math.ceil(canceledBookings.length / itemsPerPage); // Calculate total pages
  const startIndex = (currentPage - 1) * itemsPerPage; // Start index for slicing
  const currentItems = canceledBookings.slice(startIndex, startIndex + itemsPerPage); // Current page items
  ...
  return (
    <div className="p-4 bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg shadow-lg">
      <h2 className="text-xl font-semibold text-blue-800 mb-4">Canceled Bookings</h2>
      <BookingTable bookings={currentItems} showDropdown={false} /> {/* Pass the paginated bookings */}

      {/* Render pagination controls only if there are canceled bookings */}
